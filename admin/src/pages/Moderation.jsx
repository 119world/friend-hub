...
import { Ban, CreditCard, Flag, RefreshCcw, ShieldAlert } from "lucide-react";
    ...
    <section className="space-y-5">
      <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,.06)] md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">Review reported users, blocked chats, Cashfree recharge history, and refund requests.</p>
        </div>
        ...
        <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,.06)]">
          ...
          <div className="mt-4 space-y-3">
            {payments.slice(0, 6).map((payment) => {
              const id = payment.id || payment.order_id || payment.orderId;
              return (
                <div key={id} className="rounded-xl bg-[#f7f9ff] p-3 text-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-black text-slate-800">Order: {payment.order_id || payment.orderId || id}</p>
                      <p className="mt-1 font-semibold text-slate-500">INR {payment.amount || 0} | Txn: {payment.payment_id || payment.paymentId || "Pending"}</p>
                    </div>
                    <StatusPill tone={payment.status === "SUCCESS" ? "emerald" : payment.status === "FAILED" ? "rose" : "amber"}>{payment.status || "PENDING"}</StatusPill>
                  </div>
                </div>
              );
            })}
